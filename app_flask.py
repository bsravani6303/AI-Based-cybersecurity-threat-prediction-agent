...
def build_live_pdf_report(threats, stats):
    if not threats:
        return
    try:
        from reportlab.lib.pagesizes import A4
    ...
    LIVE_PDF_PATH.parent.mkdir(parents=True, exist_ok=True)
    c = canvas.Canvas(str(LIVE_PDF_PATH), pagesize=A4)
    width, height = A4
    ...
    # Packet, IP & dataset table
    c.setFont("Helvetica-Bold", 11)
    c.drawString(margin, y, "Packet, IP & Dataset Details (recent threats)")
    y -= 16
    c.setFont("Helvetica", 8)
    headers = ["Time", "Source IP", "Dest IP", "Port", "Severity", "Threat Type", "Dataset"]
    col_widths = [70, 75, 75, 28, 50, 70, 80]
    x = margin
    ...
    for t in rows:
        ...
        values = [
            ...
            str(t.get("port", "")),
            str(t.get("severity", "")),
            str(t.get("threat_type", "")),
            str(t.get("dataset", ""))[:18],
        ]
...
def download_threat_report():
    ...
    build_live_pdf_report(threat_history, threat_stats)
    if LIVE_PDF_PATH.exists():
        return send_file(
            LIVE_PDF_PATH,
            as_attachment=True,
            download_name=f"live_threat_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
        )

    df = pd.DataFrame(threat_history)
    report_path = BASE_DIR / "reports" / "threat_report.csv"
    ...
    return send_file(
        report_path,
        as_attachment=True,
        download_name=f"threat_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    )
